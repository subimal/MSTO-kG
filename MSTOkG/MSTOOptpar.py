...
def getEnergies(mol, nroots=4, fci=False, cisd=True):
    ...
    :param nroots: Number of roots for the FCI and CI solvers.
    :type nroots: int, defaults to 4.
    :param fci: Whether to compute Full Configuration Interaction energy.
    :type fci: bool, defaults to False.
    :param cisd: Whether to compute Configuration Interaction Singles and Doubles energy.
    :type cisd: bool, defaults to True.

    :returns: A dictionary containing computed energies and converged PySCF objects.
    :rtype: dict

        * **HF**: Total Hartree-Fock energy (float).
        * **mol_HF**: The converged Restricted Hartree-Fock (RHF) mean-field object.
        * **FCI**: List of FCI root energies (if fci=True).
        * **mol_FCI**: The PySCF FCI solver object (if fci=True).
        * **CISD**: Total CISD energy of the first root (float).
        * **mol_CISD**: The converged PySCF CISD object.

    :Example:

    >>> mol = moldict('sto-3g')['H2']
    >>> energies = getEnergies(mol, cisd=True)
    >>> print(f"HF Energy: {energies['HF']}")
    >>> print(f"CISD Energy: {energies['CISD']}")
    '''
...
def dumpBSI(BSI_instance, fname="BS_EO_pickled"):
    ...
    :param BSI_instance: The dictionary returned by getEnergies containing
                         'HF', 'CISD', and the molecule object.
    :type BSI_instance: dict
...
def randomize(a, fac):
    ...
    :param a: The input array (e.g., basis set coefficients).
    :type a: numpy.ndarray
    :param fac: The fluctuation scaling factor.
    :type fac: float
    :return: Perturbed array with values scaled by (1 + fac * random_offset).
    :rtype: numpy.ndarray
    '''
...
def dmol(mol, fac_basis, fac_r):
    ...
    This function acts as a high-level mutation operator, perturbing both
    the basis set exponents/coefficients and the atomic coordinates.

    :param mol: The template PySCF molecule object.
    :type mol: pyscf.gto.mole.Mole
    :param fac_basis: Scaling factor for basis set perturbations.
    :type fac_basis: float
    :param fac_r: Scaling factor for atomic coordinate perturbations.
    :type fac_r: float
    :return: A deep copy of the molecule with modified parameters.
    :rtype: pyscf.gto.mole.Mole
    '''
    from copy import deepcopy
...
def CrossOvers(pop):
    ...
    :param pop: A list of [energy, molecule] pairs representing the current population.
    :type pop: list
    :return: A list of new [energy, molecule] pairs generated via crossover.
    :rtype: list
    '''
    # The crossovers
    colst = []
...
def OptimizeElement(mol, Nmax, fname, trials=10, tol=1e-10, trials_mu=100, trials_co=100):
    ...
    :param mol: Initial PySCF molecule object used as a template.
    :type mol: :obj:`pyscf.gto.mole.Mole`
    :param Nmax: Population size of candidates for global exploration.
    :type Nmax: int
    :param fname: Path to the pickle file where the best candidate is updated.
    :type fname: str
    :param trials: Number of major optimization cycles.
    :type trials: int
    :param tol: Energy tolerance for population convergence (Hartree).
    :type tol: float
    :param trials_mu: Number of mutation attempts per major cycle.
    :type trials_mu: int
    :param trials_co: Number of crossover operations per mutation trial.
    :type trials_co: int
    :returns: Tuple of (final population, generation history, energy history).
    :rtype: tuple

    :Example:

    >>> element = 'Li'
    >>> mol = moldict('sto-3g')[element]
    >>> fname = "MOBS_Li_msto-03g.pickle"
    >>> pop, gen, en = OptimizeElement(mol, 5, fname, trials=2)
    '''
...
def AggressiveRefinement(mol, fname, trials=10, tol=1e-16, fci=True, itermax=100):
    '''
    Performs a local, gradient-free iterative search (hill-climbing) to fine-tune 
    basis set parameters. This exploitation phase provides the precision 
    necessary for scientific accuracy.

    :param mol: Best molecule object from the global search phase.
    :param fname: Path to save the refined candidate.
    :param trials: Number of independent refinement trials.
    :param tol: Energy tolerance for local convergence.
    :param fci: Whether to include FCI results in the saved output.
    :param itermax: Max iterations permitted per refinement trial.

    :Example:

    >>> mol_best = loadBSI("MOBS_Li_msto-03g.pickle")['mol_CISD'].mol
    >>> AggressiveRefinement(mol_best, "MOBS_Li_refined.pickle", trials=5)
    '''
...
def PrintEnergies2(element, ncas=None, nelecas=None, dirname = "./AtomOptimizedv3/", table=True, coeffs=False, prefix="MOBS"):

    assert ncas!=None and nelecas!=None, "Give values for ncas and nelecas."
    styles = [dict(selector="caption",
                       props=[("text-align", "center"),
                              ("font-size", "150%"),
                              ("color", 'black')])]

    columns=['k', "basis", "HF", "FCI", "CISD", "Ecorr(CISD)", "CASSCF", "NEVPT", r"'CASSCF+NEVPT"]
    df = pd.DataFrame(columns=columns)
    
    # get the STO basis done
    for k in range(2,7):
        ...
        try:
            ...
            try:
                ...
                #2s and above is auto chosen by pyscf with ncas, it's the homo. 
                #Giving ncas of 1 and nelecas of 1 means you only have 2s, so no correl. Sanity check 1. 
                #Giving ncas of 4 and nelecas of 1 still must yield no correl, almost. Sanity check 2. 
                mc.kernel() 
                en_casscf = mc.e_tot
                # print(f"en_casscf = {en_casscf}")
    
                E_nevpt2 = mrpt.NEVPT(mc).kernel()
                # print(E_nevpt2)
                # print(mc.e_tot + E_nevpt2)
            except:
                pass
                
            df.loc[len(df)] = dict(zip(columns, 
                            [k, 'STO', l['HF'], l['FCI'][0], l['CISD'], l['mol_CISD'].e_corr[0], en_casscf, E_nevpt2, en_casscf+E_nevpt2 ]))
        except:
            pass
    # get the MSTO basis done
    for k in range(2,12):
        ...
        try:
            ...
            df.loc[len(df)] = dict(zip(columns, 
                            [k, 'MSTO', l['HF'], l['FCI'][0], l['CISD'], l['mol_CISD'].e_corr[0], en_casscf, E_nevpt2, en_casscf+E_nevpt2   ]))
        except:
            pass

    for basis in ['6-31g', 'ccpvdz']:
        try:
            l = loadBSI(os.path.join(dirname, f"BS_{element}_{basis}.pickle") )
            mol = l['mol_HF'].mol
            en_casscf = '\0'
            E_nevpt2 = '\0'
            try:
            ...
            df.loc[len(df)] = dict(zip(columns, 
                            ['\0', basis, l['HF'], l['FCI'][0], l['CISD'], l['mol_CISD'].e_corr[0], en_casscf, E_nevpt2, en_casscf+E_nevpt2  ]))
        except:
            pass

    for basis in ['ccpvtz', 'ccpvqz']:
        try:
            l = loadBSI(os.path.join(dirname, f"BS_{element}_{'6-31g'}.pickle") )
            mol = l['mol_HF'].mol
            mol.basis = basis
            mol.build()
            en_casscf = '\0'
            ...
            pass
    # df = df.style.set_caption(f"Comparing MSTO basis energies (Ncas = {ncas}, Nelecas={nelecas})")
    return df


def PrintEnergies(element, dirname = "./AtomOptimizedv3/", table=True, coeffs=False, plot=False, prefix="MOBS", save=True, ylim=None, ylim_ecor=None, kmax=11, inset_loc = [0.1,0.6, 0.35, 0.35], inset_xlim=(1.9,11.1), inset_ylim=(-10,0)):
    try:
        STO = {}
        STO_ecor = {}
        for k in range(2,7):
            ...
            STO_ecor[k] = STO[k]['mol_CISD'].e_corr[0]
        ...
        for k in range(2,kmax+1):
            ...
            try:
                ...
                MSTO_ecor[k] = MSTO[k]['mol_CISD'].e_corr[0]
                # print(f"MSTO_ecor[{k}] = {MSTO_ecor[k]}")
            except:
                pass
    except:
        pass
    ...
    T2 = PrettyTable()
    T2.field_names = ['Basis', 'Ecor (CISD)']
    for k in range(2,12):
        if k in STO_ecor.keys():
            T2.add_row([f"STO-{k}G", STO_ecor[k]])
        if k in MSTO_ecor.keys():
            try:
                T2.add_row([f"MSTO-{k}G", MSTO_ecor[k][0]])
            except:
                T2.add_row([f"MSTO-{k}G", MSTO_ecor[k]])
                

    try:
        T2.add_row(["6-31G", en631g['mol_CISD'].e_corr[0]])
        T2.add_row(["CCPVDZ", enccpvdz['mol_CISD'].e_corr[0]])
    except:
        pass
    ...
    if plot:
        ...
        sto_ecor = np.array([[k, STO_ecor[k]] for k in STO_ecor.keys() ])
        ...
        msto_ecor = np.array([[k, MSTO_ecor[k]] for k in MSTO_ecor.keys() ])
        
        # try:
        #     msto_ecor = np.array([[k, MSTO_ecor[k][0]] for k in MSTO_ecor.keys() ])
        # except:
        #     msto_ecor = np.array([[k, MSTO_ecor[k]] for k in MSTO_ecor.keys() ])
            
        # print("msto_ecor = ", msto_ecor)
        # pl.ion()
        ...
        ax2.plot(sto_ecor[:,0], sto_ecor[:,1], 'o-', label='STO [CISD]', alpha=0.5)
        # print(msto_ecor)
        ax2.plot(msto_ecor[:,0], msto_ecor[:,1], 'o-', label='MSTO [CISD]', alpha=0.5)
        ...
        try:
            ...
            ax1.axhline(en631g['CISD'], color="#000000", linestyle='solid', label='6-31G (CISD)', alpha=0.5)
            ax2.axhline(en631g['mol_CISD'].e_corr[0], color="#000000", linestyle='solid', label='6-31G (CISD)', alpha=0.5)
        except:
            pass
        ...
        try:
            ...
            ax1.axhline(enccpvdz['CISD'], color="#ff00ff", linestyle='solid', label='cc-pVDZ (CISD)', alpha=0.5)
            ax2.axhline(enccpvdz['mol_CISD'].e_corr[0], color="#ff00ff", linestyle='solid', label='cc-pVDZ (CISD)', alpha=0.5)
        except:
            pass
        ...
        ax2.set_xlabel(r"$k$")
        ax2.set_ylabel(r"$E_\text{cor}$ (Ha)")
        ax2.legend(loc='right', framealpha=0.5)
        
        if ylim is not None:
            ax1.set_ylim(ylim)
            
        if ylim_ecor is not None:
            ax2.set_ylim(ylim_ecor)
        ...
        try:
            ...
            axins.axhline(en631g['CISD'], color="#000000", linestyle='solid', label='6-31G (CISD)', alpha=0.5)
        except:
            pass
        ...
        try:
            ...
            axins.axhline(enccpvdz['CISD'], color="#ff00ff", linestyle='solid', label='cc-pVDZ (CISD)', alpha=0.5)
        except:
            pass
        ...
        return fig, T1, T, T2, MSTO, MSTO_coeffs
...
def GenerateReport(dirname = "./testdata/"):
    ...
    for element in 'H Li Be B C N O F'.split(' '):
        ...
        try:
            fig, T1, T2, T3, MSTO, MSTO_coeffs = PrintEnergies(element, dirname = dirname, table=True, plot=True, prefix="MOBS", 
            ...
            with doc.create(Section(element)):
                ...
                doc.append(NoEscape(r'\vskip 1cm '))
                doc.append(NoEscape(T3.get_latex_string()))
